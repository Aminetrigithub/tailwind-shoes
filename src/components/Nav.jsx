...
<ul className=" lg:space-x-8 lg:bg-transparent lg:border-none flex flex-col  lg:flex-row  bg-gray-100 rounded-lg border p-4 border-gray-100 text-lg ">{ROUTES.map((route, i )=>{ return <li key={i} className= {`cursor-pointer rounded px-3 py-2 ${(i === 0 )? "text-white lg:bg-transparent lg:text-blue-500 bg-blue-500 " : "hover:bg-gray-400"}`}>{route}</li> }) }
</ul>
</div>

{/* Panier: cart button */}
<div className="fixed bottom-4 left-4 lg:static">
<div className="h-12 w-12 rounded-full flex-center bg-slate-400  shadow-md ">
<TbShoppingBag />
</div>
</div>
</nav>
